...
public class KafkaManagerController extends BaseController{
    ...
    * */
    ...
    @GetMapping(value = "/topic/schema/value/get")
    public RestResponse getValueTopicSchema(@RequestParam("topicName") String topicName) {

        try {
            JSONObject obj = kafkaManagerService.getValueSchema(topicName);
            return SUCCESS_DATA(obj);
        } catch (Exception e) {
            LOG.error("Get Value Topic Error,message:", e);
            return ERROR("error");
        }
    }

    @GetMapping(value = "/topic/schema/key/get")
    public RestResponse getKeyTopicSchema(@RequestParam("topicName") String topicName) {

        try {
            JSONObject obj = kafkaManagerService.getKeySchema(topicName);
            return SUCCESS_DATA(obj);
        } catch (Exception e) {
            LOG.error("Get Key Topic Error,message:", e);
            return ERROR("error");
        }
    }

    @PostMapping(value = "/topic/schema/value/create")
    public RestResponse createValueTopicSchema(@RequestParam("topicName") String topicName, @RequestBody String schema) {

        try {
            JSONObject obj = kafkaManagerService.createValueSchema(topicName, schema);
            return SUCCESS_DATA(obj);
        } catch (Exception e) {
            LOG.error("Create Value Topic Error,message:", e);
            return ERROR("error");
        }
    }

    @PostMapping(value = "/topic/schema/key/create")
    public RestResponse createKeyTopicSchema(@RequestParam("topicName") String topicName, @RequestBody String schema) {

        try {
            JSONObject obj = kafkaManagerService.createKeySchema(topicName, schema);
            return SUCCESS_DATA(obj);
        } catch (Exception e) {
            LOG.error("Create Key Topic Error,message:", e);
            return ERROR("error");
        }
    }

    @GetMapping(value = "/topic/schema/value/version")
    public RestResponse getValueTopicSchemaVersion(@RequestParam("topicName") String topicName) {

        try {
            JSONObject obj = kafkaManagerService.getValueSchemaVersion(topicName);
            return SUCCESS_DATA(obj);
        } catch (Exception e) {
            LOG.error("Get Value Topic Version Error,message:", e);
            return ERROR("error");
        }
    }

    @GetMapping(value = "/topic/schema/key/version")
    public RestResponse getKeyTopicSchemaVersion(@RequestParam("topicName") String topicName) {

        try {
            JSONObject obj = kafkaManagerService.getKeySchemaVersion(topicName);
            return SUCCESS_DATA(obj);
        } catch (Exception e) {
            LOG.error("Get Key Topic Version Error,message:", e);
            return ERROR("error");
        }
    }
}
